...
import { calculateTokenAmount } from "./lib/utils";
...
function App() {
  ...
  const handleSellAmountChange = (value: string) => {
    ...
    if (sellToken && buyToken) {
      const buyAmount = calculateTokenAmount(value, sellToken, buyToken);
      form.setValue("buyAmount", buyAmount, { shouldValidate: true });
    }
  };
  ...
  const handleBuyAmountChange = (value: string) => {
    ...
    if (sellToken && buyToken) {
      const sellAmount = calculateTokenAmount(value, buyToken, sellToken);
      form.setValue("sellAmount", sellAmount, { shouldValidate: true });
    }
  };
  ...
  const handleTokenSelect = (token: Token) => {
    ...
    if (activePanel === "sell") {
      ...
      if (sellAmount && buyToken) {
        const buyAmount = calculateTokenAmount(sellAmount, token, buyToken);
        form.setValue("buyAmount", buyAmount, { shouldValidate: true });
      }
    } else {
      setBuyToken(token);
      ...
      if (buyAmount && sellToken) {
        const sellAmount = calculateTokenAmount(buyAmount, token, sellToken);
        form.setValue("sellAmount", sellAmount, { shouldValidate: true });
      }
    }
    setSearchQuery("");
      ...
      <div className="bg-zinc-900 rounded-2xl w-full max-w-lg p-6 relative shadow-lg flex flex-col items-end top-20">
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="w-full">
            ...
            <Button
              type="submit"
              className={`bg-[#14f195] text-black border-none rounded-xl text-base font-semibold py-4 w-full mt-6 cursor-pointer transition-colors duration-200 hover:bg-[#0dd584] ${
